...
class VbeeAuto:
    ...
    def tts(self, asset_token, text, voice, speech, punctuation, log_callback=print):
        punctuation = punctuation.split(',')
        try:
            # Convert to float and round to 2 decimal places
            sentence_break = round(float(punctuation[0]), 2)
            medium_break = round(float(punctuation[1]), 2)
            major_break = round(float(punctuation[2]), 2)
            paragraph_break = round(float(punctuation[3]), 2)
            ...
            data = {"audioType": "mp3", "bitrate": 128, "backgroundMusic": {"volume": 80}, "text": text, "voiceCode": voice,
                    "speed": speech, "datasenses": None, "clientPause": {"sentenceBreak": sentence_break, "mediumBreak": medium_break, "majorBreak": major_break, "paragraphBreak": paragraph_break}}  
            print("data", data)
            response = requests.post(url, json=data, headers=headers)
    ...
    def check_bonus_characters(self, access_token, log_callback=print):
        ...
        bonus_characters = json.loads(response.text)[
            "result"]["bonus_characters"]
    ...
    def tts_from_text(self, access_token, content, voice="hn_female_ngochuyen_full_48k-fhg", speed=1.0, save_path="", account_id=0):
        ...
        while retry_count < max_retries:
            res = requests.post(url=synthesis_url,
        ...
        while audio_retry_count < max_audio_retries:
            audio_response = requests.get(
                url=f'https://vbee.vn/api/v1/requests/{request_id}/audio', headers=headers)
            print(
                f"audio_response lần {audio_retry_count + 1}: {audio_response.status_code}")
                    ...
                    if response_json.get("status") == 1 and "result" in response_json and "audio" in response_json["result"]:
                        ...
                        if audio_file_response.status_code == 200:
                            ...
                            if save_path:
                                ...
                                print(
                                    f"Đã lưu file âm thanh thành công tại: {save_path}")
                                return save_path
                            else:
                                return audio_file_response.content
                        else:
                            print(
                                f"Lỗi khi tải audio file: {audio_file_response.status_code}")
                    else:
            ...
            if audio_retry_count < max_audio_retries:
                print(
                    f"Lần {audio_retry_count}: Audio chưa sẵn sàng, thử lại sau 3 giây...")
                time.sleep(3)  # Đợi 3 giây trước khi thử lại
            else:
                print(
                    f"Đã thử {max_audio_retries} lần nhưng không lấy được audio")
                break
